 ...
 * Если в методе для потока А срабатывает wait(), то поток уходит в wait-set,
 * метод помечается как свободный и готов для других потоков.
 * Если вызывается nitify(), то случайный поток из wait-set попадает в blocked-set,
 * и ожидает своей очереди входа в метод.
 */
